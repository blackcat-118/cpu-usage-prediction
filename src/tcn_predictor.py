...
class TCNPredictor:
    """TCN-based predictor with API compatible to TimesfmPredictor/LSTMPredictor (minus finetune)."""
    ...
    def __init__(self, pod_name: str, context_len: int = 64, pred_len: int = 8, pretrained_path: str = "pretrained_data_3.csv", use_features: bool = True):
        ...
        self.log_list = ["src/api.py", "src/tcn_predictor.py"]
        ...
        self._wandb_init()
        # self._train_from_pretrained()

    # ------------------------------------------------------------------
    # Setup / training
    # ------------------------------------------------------------------
    def _wandb_init(self):
        self.wb_runner = wandb.init(project=wandb_project, group="exp1_tcn", name=f"tcn-{self.pod_name}", reinit="create_new")
    ...
    def terminate(self):
        ...
        if self.wb_runner is not None:
            ...
            # only log files within repository root; include_fn guards the set
    ...
    def _build_model(self):
        """Build and compile a TCN model using keras-tcn library with multi-feature input.

        Input shape: (context_len, feature_dim), feature_dim = 1 (cpu) + len(covariates).
        Output: pred_len future cpu values (Dense(pred_len)).
        """
        feature_dim = len(self.covariate_keys) + 1
        inp = Input(shape=(self.context_len, feature_dim))
        
        # Use keras-tcn TCN layer with dilated causal convolutions
        tcn_out = TCN(
            nb_filters=64,
            kernel_size=3,
            nb_stacks=1,
        ...
        x = Dense(128, activation="relu")(tcn_out)
        x = Dropout(0.2)(x)
        out = Dense(self.pred_len)(x)
    ...
    def train_from_pretrained(self):
        ...
        for i in range(len(values) - self.context_len - self.pred_len):
            ...
            future_cpu = values[i + self.context_len:i + self.context_len + self.pred_len, 0]
        ...
        print(f"Training TCN on {X_train.shape[0]} sequences; validating on {X_val.shape[0]} sequences.")
    ...
    def plot(self):
        ...
        plt.title("CPU Usage Forecast (TCN)")
    ...
    def plot_evaluation(self):
        ...
        plt.grid(True); plt.title("Prediction Evaluation (MAE)")
        plt.xlabel("Time"); plt.ylabel("MAE"); plt.legend()
        plt.ylim(0, 30)
        plt.gca().xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
        plt.gca().xaxis.set_major_locator(mdates.AutoDateLocator(maxticks=10))
        plt.gcf().autofmt_xdate(rotation=45)
        plt.tight_layout(); plt.savefig(f"{self.log_folder}/evaluation.png"); plt.close()
